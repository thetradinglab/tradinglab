...
contract ReferralV1 is Ownable, ReentrancyGuard, Initializable {
    ...
    function register(address referrer) external nonReentrant whenNotPaused allContractsConnected {
        ...
        try rewardToken.safeTransferFrom(msg.sender, address(this), registrationAmount) {
    ...
    function subscribe(address user) external nonReentrant whenNotPaused allContractsConnected {
        ...
        // External calls last
        try rewardToken.safeTransferFrom(user, address(this), subscriptionAmount) {
     ...
     * @param user Address of the user
     * @return User stats including referrer, counts, and status
     */
    ...
    function getReferralTree(
        ...
        if (maxReferralDepth >= 2) {
            uint256 level1Count = level1.length > maxReferralsPerLevel ? maxReferralsPerLevel : level1.length;
